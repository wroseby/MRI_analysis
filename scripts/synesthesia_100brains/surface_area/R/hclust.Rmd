...
hc = merge(parcel_positions, hc[[1]]) # merge with 2D coordinates
...
write.csv(hc, file = paste0(savepath_data, 'hclust/hc.csv'))
```
...
## TESTING METRICS WITH NORMALISED WEIGHTS / WEIGHT-PRESERVING RANDOM MODELS - DOES TOPOLOGY MATTER? ##
...
gam_model = gam(weight ~ s(distance, by = Group, k = 7) + Group, data = dists,
             family = 'gaussian')
print(summary(gam_model))  # for signifiance of model terms
gam_model_no_int = gam(weight ~ s(distance, k = 7) + Group, data = dists, 
                       family = 'gaussian')  # without group separation
print(anova.gam(gam_model, gam_model_no_int, 
          test = 'Chisq'))  # for testing the group separation

# Predict weight-distance relationship from fitted GAM
distance_grid = seq(min(dists$distance), max(dists$distance), length.out = 500)
dist_tem_cntrl = data.frame(distance = distance_grid, 
                            Group = factor("Control", 
                                           levels = levels(dists$Group)))
dist_tem_exp = data.frame(distance = distance_grid, 
                               Group = factor(exp_group, 
                                              levels = levels(dists$Group)))
weight_pred_cntrl = predict(gam_model, 
                            newdata = dist_tem_cntrl, type = "response")
weight_pred_exp = predict(gam_model, 
                            newdata = dist_tem_exp, type = "response")
GAM_preds = data.frame(
  distance = rep(distance_grid, 2),
  weight = c(y_pred_control, y_pred_treatment),
  Group = rep(c("Control", "Treatment"), each = length(distance_grid))
)

# Plot predicted curves
ggplot(GAM_preds, aes(x = distance, y = weight, color = Group)) +
  geom_line(size = 1.2) +
  
  scale_color_manual(values = c("blue", "red"))+
  
  labs(title = "Predicted GAM curves", 
       x = 'Two-dimensional distance (pixels)',
...
# Plot predicted difference curve
GAM_diffs = data.frame(
  distance = distance_grid,
  difference = weight_pred_cntrl - weight_pred_exp
)
ggplot(GAM_diffs, aes(x = distance, y = difference)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  geom_line(color = "darkgreen", size = 1.2) +
  
  labs(title = "Group difference in GAM curves", 
     x = 'Two-dimensional distance (pixels)',
     y = "Control - Treatment")+
  theme_classic()+
  theme(
...
# Get numerical curve differences
mean_difference = mean(GAM_diffs$difference)
max_difference = max(abs(GAM_diffs$difference))

cat("Mean difference across distance:", mean_difference, "\n")
cat("Maximum absolute difference:", max_difference, "\n")
```
...
# Set filename for split network metrics --- change if needed
split_metrics_filename = 'hc_matched_split_net_metrics.csv' 
net_metrics = read.csv(paste0(savepath_data,
                              'hclust/', split_metrics_filename))
...
# Visualise weight-distance relationships for split networks #

# Import and combine 2D distance data
dists_cntrl_filename = 'hc_matched_cntrl_split_dists.csv'  # filename for control distances --- change if needed
dists_exp_filename = 'hc_matched_syn_split_dists.csv'  # filename for experimental distances --- change if needed
