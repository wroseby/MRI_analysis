...
pcorsolve = function(data){
  covmat = cov(data)
  premat = solve(covmat)
  D = sqrt(diag(premat))
  partial_corr = (-premat / (D %o% D))
  return(partial_corr)
}
...
plot_pcors = function(data_list, nclust, group_vec, colours = c('blue', 'red')){
...
sampleCopula = function(data, log = F, 
                        copulatype = c('gaussian', 'empirical', 't', 'vine'), invert = c('gaussian', 'empirical'),
                        n = 102, n_samp = 10){
  ...
  if (copulatype == 'vine'){
    # VineCopula package:
    # copula = RVineStructureSelect(pseudoobs, familyset = c(1,2,)) 
    # rvinecopulib package
    copula = vinecop(pseudoobs, family_set = c("elliptical"), cores = 3)
  }
  ...
  for (i in 1:n_samp){
    if (copulatype == 'vine'){
      # VineCopula package:
      #copula_samples[[i]] = RVineSim(n, copula)
      # rvinecopulib package:
      copula_samples[[i]] = rvinecop(n, copula)
    }
    else{
    ...
    # no inversion - uniform samples
    if (invert == 'none'){
      samples = copula_samples[[i]]
    }
...
# Function to calculate graph complexity based on pairwise Jensen-Shannon divergence
js_complexity = function(data, group, randomise = F) {
  # Helper functions
  normalize = function(x) { # normalise distributions to sum to 1
    total = sum(x)
    if (total == 0) return(rep(1 / length(x), length(x)))  # Handle zero-sum case
    return(x / total)
  }
  kl_divergence = function(P, Q) { # Kullback-Leibler divergence
    nonzero = P > 0 # to avoid logging 0
    sum(P[nonzero] * log2(P[nonzero] / Q[nonzero]))
  }
  js_divergence = function(P, Q) { # Jensen-Shannon divergence
    M = 0.5 * (P + Q)
    0.5 * kl_divergence(P, M) + 0.5 * kl_divergence(Q, M)
  }
  
  # Wrangle data
  data = data[data$Group == group, 8:ncol(data)] # get desired group and variables
  edge_weights = pcorsolve(data) # get partial correlations
  if (randomise == T){ # if we want to randomly reorganise graph
    set.seed(params$seed)
    shuffled_weights = sample(as.vector(edge_weights)) # randomly shuffle edge weights
    edge_weights = matrix(shuffled_weights, nrow = nrow(edge_weights)) # put back into adjacency matrix
  }
  edge_weights = abs(edge_weights) # get absolute partial correlations
  prob_matrix = t(apply(edge_weights, 1, normalize)) # normalise distributions
  
  
  # Get JS divergence between every vertex
  n = nrow(prob_matrix)
  js_matrix = matrix(0, n, n)
  for (i in 1:(n - 1)) {
    for (j in (i + 1):n) {
      js_val = js_divergence(prob_matrix[i, ], prob_matrix[j, ])
      js_matrix[i, j] = js_val
      js_matrix[j, i] = js_val
    }
  }
  
  # Get mean vertex- and graph-wise divergences
  vertex_means = rowMeans(js_matrix)
  total_js = mean(js_matrix)

  return(list(
    js_matrix = js_matrix,
    vertex_means = vertex_means,
    total_js = total_js
  ))
}
...
# Write clustering to csv
write.csv(clust_wrd_pos, file = paste0(savepath_data, '/hclust/clust_wrd_pos.csv'))
...
To make sure our examination of correlations reflects simply the group differences in correlation structure, rather than differences in regional size, we want to ensure that we correct for any differences in the size of each ROI. We'll examine this first for the lr-averaged parcels, and also for the identified clusters, by fitting linear models to compare groups at each region:

```{r linear_model, warning=F, message=F}
# Create new dataframe for linear model residuals for lr-averaged parcels
SA_abs_lr_lm = SA_abs_lr[, c('ID', 'Age', 'Sex', 'Group', 'Subgroup', 'Scan', 'Partition')]  

# Fit model for each area and store result
model_summaries = list()
for (region in colnames(SA_abs_lr[, 8:ncol(SA_abs_lr)])) {  
  model = lm(SA_abs_lr[[region]] ~ Group, data = SA_abs_lr)
  model_summaries[[region]] = summary(model)
  SA_abs_lr_lm[[paste0(region)]] = residuals(model)  
}

print(head(SA_abs_lr_lm))

# Check for any significant differences
sapply(model_summaries, FUN = function(list){
  list[["coefficients"]][2,4]
})

#SA_abs_lr = SA_abs_lr_lm # replace data - uncomment if desired

# Summarise lr data into clusters
SA_abs_cntrl_wrd = rbind(summarise_clusters(SA_abs_lr[SA_abs_lr$Group == 'Control',], clust_wrd_pos)[[1]],
                            summarise_clusters(SA_abs_lr[SA_abs_lr$Group == 'Syn',], clust_wrd_pos)[[1]])

# Create new dataframe for linear model residuals for clusters
SA_abs_cntrl_wrd_lm = SA_abs_cntrl_wrd[, c('ID', 'Age', 'Sex', 'Group', 'Subgroup', 'Scan', 'Partition')]

# Fit model for each cluster and store result
model_summaries = list()
for (cluster in colnames(SA_abs_cntrl_wrd[, 8:ncol(SA_abs_cntrl_wrd)])) {  
  model = lm(SA_abs_cntrl_wrd[[cluster]] ~ Group, data = SA_abs_cntrl_wrd)
  model_summaries[[cluster]] = summary(model)
  SA_abs_cntrl_wrd_lm[[paste0(region)]] = residuals(model)  
}
...
Because there are no differences in region size between the groups that exceed any variation within the groups, for either lr-averaged parcels or clusters, we can use the raw surface area data. 
...
Analysis in python shows that there are indeed the expected differences between the macthed synesthete and control samples in terms of basic network metrics. 
...
These results confirm that there are clear differences between controls and synesthetes in their correlation structure. We now want to extend our analysis to include the examine the role of physical distance.
...
While the control SWP is around what you would expect for a brain-derived network, the synesthete SWP is lower, indicating less small-world properties. The delta values indicate that while both networks are closer to their random null models, in synesthetes this is particularly extreme, suggesting a substantial deviation from local clustering.
...
knitr::include_graphics(paste0(python_imports, 'SA_abs_matched_clust_cntrl_bcnet.png'))
```

`
```{r bcnetwork_syn, fig.cap = 'Synesthete network for vertex colour and size scale with clustering and betweenness'}
knitr::include_graphics(paste0(python_imports, 'SA_abs_matched_clust_syn_bcnet.png'))
```

Along with the global differences in clustering, we see some more specific differences. Vertex betweenness is more variable for synesthetes than controls, with some vertices (such as the PCC) having very high betweenness. Vertices with higher betweenness also seem to have greater clustering, although this isn't always the case.

Note that this can be difficult to interpret since betweenness and clustering will be overall greater when there are stronger edges (which there are in synesthetes). Note - we should compare the distributions of the absolute values of these metrics, since the scaling for visualisation could also be misleading.

To perhaps improve the visualisation, we also created a network showing the differences between the control and synesthete networks. Node colour and size correspond to differences in local clustering and betweenness, while edge size corresponds to the size of the difference between partial correlation size. All differences are absolute. 

```{r diffnetwork, fig.cap = 'Network showing group differences in correlation strength, local clustering and betweenness'}
knitr::include_graphics(paste0(python_imports, 'SA_abs_matched_clust_diffnet.png'))
```
...
So far, we have been looking at networks formed from absolute partial correlation, under the assumption that any association is important for distinguishing groups. However, there may be specific information contained within the positive or negative associations alone. To address this, we'll examine networks formed from just the positive or negative correlations.

First, let's compare the distributions of positive and negative correlations across groups:

```{r signed_pcor_distributions, fig.cap = 'Distributions of positive and negative partial correlations'}
# Generate partial correlations
pcor_list = lapply(SA_abs_matched_clust, FUN = function(df){pcor(df[,8:ncol(df)])$estimate})
pcor_list = lapply(pcor_list, FUN = function(df){df = melt(df)})
...
knitr::include_graphics(paste0(python_imports, 'SA_abs_matched_clust_cntrl_split_pos.png'))
```

```{r split_net_control_neg, fig.cap = 'Network of negative partial correlations in controls'}
knitr::include_graphics(paste0(python_imports, 'SA_abs_matched_clust_cntrl_split_neg.png'))
```

```{r split_net_syn_pos, fig.cap = 'Network of positive partial correlations in synesthetes'}
knitr::include_graphics(paste0(python_imports, 'SA_abs_matched_clust_syn_split_pos.png'))
```

```{r split_net_syn_neg, fig.cap = 'Network of negative partial correlations in synesthetes'}
knitr::include_graphics(paste0(python_imports, 'SA_abs_matched_clust_syn_split_neg.png'))
```
...
Splitting the partial correlations by positive and negative, we see that the overall relationship between weight and distance we observed previously is largely constrained to the positive networks. In the positive network, synesthetes show weaker correlations at a shorter distance, and possibly stronger correlations at long distances compared to controls. However, in the negative network, there is no obvious relationship between distance and weight in controls, while in synesthetes, there is a trend towards decreased correlation magnitude at longer distances.  

It would be a good to get an idea of the effect sizes and variance for the network metric differences. We have also created split networks for the copula samples, so let's import those results:

```{r split_nets_copula, warning=F, message=F}
# Import metrics
net_metrics = read.csv(paste0(savepath_data, 
                              'hclust/clust_wrd_matched_split_copula_net_metrics.csv'))

# Get cohen's d for control-syn comparisons for pos and neg networks
net_metrics$Sign = factor(net_metrics$Sign, levels = c('Positive', 'Negative'))
cohensd = net_metrics %>%
  group_by(Sign) %>%
  summarise(across(where(is.numeric), ~ cohens_d(.x ~ Group)$Cohens_d, .names = "d_{.col}"))
print(cohensd)

# Get summary statistics
net_metrics = melt(net_metrics, .id_vars = c('Group', 'Sign'))
net_metrics = net_metrics %>%
  group_by(Group, Sign, variable) %>%
  summarise(value_mean = mean(value), value_sd = sd(value))

# Plot results
ggplot(data = net_metrics, aes(x = Sign, y = value_mean, fill = Group))+
  geom_col(color = 'black', alpha = 0.5, position = position_dodge())+
...
These results suggest that the differences between controls and synesthetes are generally larger for the negative networks than the positive networks. Additionally, the variance of control statistics like mean correlation is actually quite similar between positive and negative networks, suggesting that these differences represent global changes in correlation strength by sign rather than contributions of a few outliers. 
...
js_complexity_control = js_complexity(SA_abs_matched_clust$Control, 'Control')
js_complexity_syn = js_complexity(SA_abs_matched_clust$Syn, 'Syn')
...
js_complexity_control_rand = js_complexity(SA_abs_matched_clust$Control, 
                                           'Control', randomise = T)
js_complexity_syn_rand = js_complexity(SA_abs_matched_clust$Syn, 
                                       'Syn', randomise = T)
